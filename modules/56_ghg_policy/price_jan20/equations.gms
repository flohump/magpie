...
*' Third, an annuity factor (annuity due with infinite time horizon) is used to obtain average annual rewards
 ...
 q56_reward_cdr_aff(j2) ..
            	 ...
            	 (
            	 sum(ac, ((1-s56_buffer_aff) * vm_cdr_aff(j2,ac) * sum((cell(i2,j2),ct), p56_c_price_aff(ct,i2,ac)))
            	 / ((1+sum(cell(i2,j2),pm_interest(i2)))**(ac.off*5)))$(s56_payment=0)
            	 +
            	 (sum(ac, (1-s56_buffer_aff) * vm_cdr_aff(j2,ac)) * sum((cell(i2,j2),ct), p56_c_price_aff(ct,i2,"ac0")))$(s56_payment=1)
            	 +
            	 ((sum(ac, (1-s56_buffer_aff) * vm_cdr_aff(j2,ac)) * sum((cell(i2,j2),ct), sum(ac$(ac.off*5=s32_planing_horizon), p56_c_price_aff(ct,i2,ac))))
            	 / ((1+sum(cell(i2,j2),pm_interest(i2)))**(s32_planing_horizon)))$(s56_payment=2)
            	 +
            	 (sum(ac, (0.7 * (1-s56_buffer_aff) * vm_cdr_aff(j2,ac) * sum((cell(i2,j2),ct), p56_c_price_aff(ct,i2,ac)))
            	 / ((1+sum(cell(i2,j2),pm_interest(i2)))**(ac.off*5))) + 
				 (sum(ac, 0.3 * (1-s56_buffer_aff) * vm_cdr_aff(j2,ac)) * sum((cell(i2,j2),ct), sum(ac$(ac.off*5=s32_planing_horizon), p56_c_price_aff(ct,i2,ac))))
            	 / ((1+sum(cell(i2,j2),pm_interest(i2)))**(s32_planing_horizon)))$(s56_payment=3)
            	 )
