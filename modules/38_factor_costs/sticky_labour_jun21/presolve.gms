...
*' Estimate capital stock based on capital remuneration
  p38_capital_immobile_t(j,kcr)   = sum(cell(i,j), i38_capital_need(i,kcr)*s38_immobile*p38_croparea_start(j,kcr)*f38_region_yield(i,kcr)* sum(supreg(h,i),fm_tau1995(h)));
  p38_capital_mobile_t(j)         = sum((cell(i,j),kcr), i38_capital_need(i,kcr)*(1-s38_immobile)*p38_croparea_start(j,kcr)*f38_region_yield(i,kcr)* sum(supreg(h,i),fm_tau1995(h)));
