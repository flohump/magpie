...
*' The conceptual and analytical details of the CES function including the labour productivity factor are documented in @orlov_ces_2021.
 ...
 q38_ces_prodfun(j2,kcr) ..
  i38_scale(j2,kcr) * 
  [i38_sh(j2,kcr)*v38_capital_need(j2,kcr)**(-s38_ep) + 
  (1 - i38_sh(j2,kcr))*(vm_labor_prod(j2) * v38_labour_need(j2,kcr))**(-s38_ep)]**(-1/s38_ep)
  =e= 1 ;
...
q38_cost_prod_crop(i2,kcr).. vm_cost_prod(i2,kcr)
                              =e= sum(cell(i2,j2), vm_prod(j2,kcr) * v38_labour_need(j2,kcr) * s38_wage / (1-s38_mi_start))
                                ;
...
q38_investment_immobile(j2,kcr).. v38_investment_immobile(j2,kcr)
                                  =g=
                                 vm_prod(j2,kcr) * v38_capital_need(j2,kcr) * s38_immobile -
...
q38_investment_mobile(j2).. v38_investment_mobile(j2)
                             =g=
                             sum(kcr, vm_prod(j2,kcr) * v38_capital_need(j2,kcr) * (1-s38_immobile)) -
