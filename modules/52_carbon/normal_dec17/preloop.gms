...
* Cap lambda at the biomass expansion factor: lambda > BEF would harvest more stemwood than the
* stand aboveground biomass holds. It arises where FRA growing stock exceeds the LPJmL biomass
* (arid MEA; managed forest in CHA IND USA REF). The hard cap keeps wood and carbon consistent;
* the capped regions then no longer reproduce FRA growing stock.
